...
int main()
{
    ...
    if (rank == p - 1)
    {
        elementsPerProcess += row1 % p
    }

    int startPoint = my_rank * elementsPerProcess;
    int endPoint = startPoint + elementsPerProcess;
    int *pArrayNum1 = (int *)malloc((endPoint - startPoint) * sizeof(int));
    int *pArrayNum2 = (int *)malloc((endPoint - startPoint) * sizeof(int));
    int *pArrayNum3 = (int *)malloc((endPoint - startPoint) * sizeof(int));

    // STEP 2: Send relevant portions the arrays to all other MPI processess in the group
    clock_gettime(CLOCK_MONOTONIC, &startComm);
    MPI_Scatter(pArrayNum1, elementsPerProcess, MPI_INT, pArrayNum1, endPoint - startPoint, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Scatter(pArrayNum2, elementsPerProcess, MPI_INT, pArrayNum2, endPoint - startPoint, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_GATHER(pArrayNum3, elementsPerProcess, MPI_INT, )
